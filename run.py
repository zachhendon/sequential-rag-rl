...
from reticl.datasets.gsm8k import GSM8K_CONFIG
from reticl.datasets.qasc import QASC_CONFIG
from reticl.datasets.commonsense_qa import CQA_CONFIG
from reticl.datasets.ecqa import ECQA_CONFIG
from reticl.datasets.ag_news import AGNEWS_CONFIG
...
def get_dataset_config(options_dict: dict) -> DatasetConfig:
    ...
    if options.dataset == Datasets.QASC.value:
        return QASC_CONFIG
    if options.dataset == Datasets.MTOP.value:
        return MTOP_CONFIG
    if options.dataset == Datasets.CQA.value:
        return CQA_CONFIG
    if options.dataset == Datasets.ECQA.value:
        return ECQA_CONFIG
...
def main():
    ...
    parser.add_argument("--corpus_size", type=int, help="Number of samples to use for training corpus; set to 0 to use all available samples")
    ...
    parser.add_argument("--val_every", type=int, help="Number of batches to train on before running validation")
    ...
    parser.add_argument("--int_reward", action="store_true", help="Get intermediate reward")
