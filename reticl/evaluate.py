...
class EvaluateSeqRAG:
    def __init__(self, generator: VLLMGenerator, dataset_config: DatasetConfig):
        self.generator = generator
        self.dataset_config = dataset_config

    def evaluate(self, split: str, options_dict: dict):
        options = TrainOptions(options_dict)
        if options.model_name:
            retriever = retriever_model(options)
            retriever.load_state_dict(
                torch.load(f"{options.model_name}.pt", map_location=device)
            )
        else:
            retriever = None
        self._evaluate_reticl(None, retriever, split, options)

    def _evaluate_reticl(
        self, run, retriever: Optional[Retriever], split: str, options: TrainOptions
    ):
        with torch.no_grad():
            ...
            if retriever:
                retriever.eval()
            dataset = RetICLDataset(self.dataset_config, split, retriever, options)
            dataset.set_greedy(
                True
            )  # Use greedy sampling for policy-based example retrieval

            # Collect predictions and labels over the dataset
            self.generator.reset_pbar(len(dataset), "Testing")
            if options.sm == SamplingMethod.EXHAUSTIVE.value:
                prompts, labels, meta_datas, preds, example_set, examples_per = (
                    self._exhaustive_eval(dataset, options)
                )
            else:
                prompts, labels, meta_datas, preds, example_set, examples_per = (
                    self._policy_eval(dataset, options)
                )

            if self.dataset_config.get("check_correct_batch"):
                correct = self.dataset_config["check_correct_batch"](
                    meta_datas, preds
                ).numpy()
            else:
                correct = np.array(
                    [
                        self.dataset_config["check_correct"](meta_data, pred)
                        for meta_data, pred in zip(meta_datas, preds)
                    ]
                )
            ...
            if options.dataset == Datasets.TABMWP.value:
                mc_acc = (
                    correct[
                        [
                            meta_data["ques_type"] == "multi_choice"
                ...
                print(
                    f"Accuracy: {acc:.2f}, MC: {mc_acc:.2f}, Free: {free_acc:.2f}, Examples Total: {len(example_set)}, Examples Per: {examples_per:.2f}"
                )
            else:
                print(
                    f"Accuracy: {acc:.2f}, Examples Total: {len(example_set)}, Examples Per: {examples_per:.2f}"
                )
            if run:
            ...
            generator_model = (
                options.gpt3_model
                if options.generator_model == "gpt3"
                else options.generator_model.replace("/", "-")
            )
    ...
    def _exhaustive_eval(self, dataset: RetICLDataset, options: TrainOptions):
        ...
        for sample in dataset.data:
            ...
            for prompt_idx in range(0, len(all_prompt_cands), options.batch_size):
                prompt_cands = all_prompt_cands[
                    prompt_idx : prompt_idx + options.batch_size
                ]
                pred_cands = [
                    pred["text"]
                    for pred in self.generator.generate(prompts=prompt_cands)
                ]
                if self.dataset_config.get("check_correct_batch"):
                    correct = self.dataset_config["check_correct_batch"](
                        [meta_datas[-1]] * len(pred_cands), pred_cands
                    ).tolist()
                else:
                    correct = [
                        self.dataset_config["check_correct"](meta_datas[-1], pred)
    ...
    def _policy_eval(self, dataset: RetICLDataset, options: TrainOptions):
        prompts: List[str] = []
        labels: List[str] = []
        meta_datas: List[dict] = []
        preds: List[str] = []
        ...
        for batch in data_loader:
            ...
            for pred in self.generator.generate(batch["prompts"]):
